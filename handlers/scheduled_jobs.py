...
def setup_scheduled_jobs(application: Application) -> None:
    ...
    jq.run_daily(job_quran_verse,      time=time(hour=13, minute=0,  tzinfo=BEIRUT_TZ))
    jq.run_daily(job_quran_verse,      time=time(hour=14, minute=0,  tzinfo=BEIRUT_TZ))
    jq.run_daily(job_quran_verse,      time=time(hour=15, minute=0,  tzinfo=BEIRUT_TZ))
    ...
    jq.run_daily(job_evening_reminder, time=time(hour=23, minute=0,  tzinfo=BEIRUT_TZ))
